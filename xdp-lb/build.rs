...
/// This crate has a runtime dependency on artifacts produced by the `xdp-lb-ebpf` crate.
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "xdp-lb-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `xdp-lb-ebpf`, so cargo will automatically rebuild us
        // if `xdp-lb-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `xdp-lb-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("xdp-lb-ebpf");
